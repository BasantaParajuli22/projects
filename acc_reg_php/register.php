...
if($_SERVER['REQUEST_METHOD']=="POST"){
    ...
    $hashed_password = password_hash($password, PASSWORD_BCRYPT);//hasing the pasword
   ...
   if(!filter_var($email,FILTER_VALIDATE_EMAIL)){
    ...
    $sql1 ="SELECT *FROM register_table WHERE username = ? OR email = ?";
    $stmt1 = $conn->prepare($sql1);
    if($conn->prepare($sql1)==false){
    ...
    if($stmt1->num_rows()>0){
        ...
        $sql = "INSERT INTO register_table (username,email,password) VALUES (?,?,?)";
